...
bool do_vector_fitting(std::vector<std::complex<double>> &p, std::vector<std::complex<double>> &r, double &rem, int nump, int numi, bool vflog)
{
  ...
  if (vflog)
  {
    try
    {
      std::filesystem::remove("vf_log.txt");
    }
    catch (...)
    {
    }
  ...
  if (vflog)
    log_file.open("vf_log.txt", std::ios::app | std::ios::out);
  std::time_t in_time = std::chrono::system_clock::to_time_t(start);
  if (log_file)
    log_file << "VECTOR FITTING LOG FILE FOR TR RUN AT : " << std::ctime(&in_time) << "\t====================================\n";
  ...
  if (log_file)
    log_file << std::ctime(&in_time) << "\tReading in S-param data...\n";
      ...
      else
      {
        ...
        while (c = row[0])
        {
          ...
          else
          {
            ...
            if (i == 1)
              freqs_hz.push_back(atof(t.c_str()));
            else if (i == 2)
              real = atof(t.c_str());
            else if (i == 3)
              imag = atof(t.c_str());
  ...
  {
    if (log_file)
      log_file << "\tS-param data not available! Make sure there is a file named 's_param_data.txt' in the same directory...\n\tExiting run...\n";
    if (log_file)
      log_file.close();
    return 0;
  }
  ...
  if (vflog)
  {
    try
    {
      std::filesystem::remove("pole_guess.txt");
    }
    catch (...)
    {
    }
  ...
  if (vflog)
    pole_write.open("pole_guess.txt", std::ios::app | std::ios::out);
  if (pole_write)
  {
  ...
  in_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
  if (log_file)
    log_file << std::ctime(&in_time) << "\tRunning Vector Fitting algorithm with " << num_poles << " starting poles for " << iters << " iterations...\n";
  ...
  if (vflog)
  {
    try
    {
      std::filesystem::remove("residual_log.txt");
    }
    catch (...)
    {
    }
  ...
  for (int iteration = 0; iteration <= iters; iteration++)
  {
    ...
    {
      if (x.imag() == 0)
        real_poles.push_back(x);
      else
        comp_poles.push_back(x);
    }
    ...
    for (int j = 0; j < Az_real_r * Az_real_c; j++)
      Az_real[j] = 0;
    for (int j = 0; j < Az_comp_r * Az_comp_c; j++)
      Az_comp[j] = 0;
    for (int j = 0; j < bz_comp_r * bz_comp_c; j++)
      bz_comp[j] = 0;
    ...
    in_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    if (log_file)
      log_file << std::ctime(&in_time) << "\tIteration " << iteration << '\t' << num_rp << '\t' << num_cp << '\n'; // FOR DEBUGGING
    ...
    for (int j = 0; j < num_freqs; j++) // rows
    {
      ...
      for (int k = 0; k < 2 * (num_rp + 2 * num_cp) + 1; k++) // cols
      {
        if (k >= 0 && k < num_rp) // LHS real entries
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in LHS real\nExiting run..." << std::endl;
            if (log_file)
              log_file.close();
            if (pole_write)
              pole_write.close();
        ...
        else if (k >= num_rp && k < num_rp + num_cp) // LHS comp poles, corresponding to real part of residue for the pair
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in LHS comp 1 with pole_iter of " << pole_iter << std::endl;
            if (log_file)
              log_file.close();
            if (pole_write)
              pole_write.close();
        ...
        else if (k >= num_rp + num_cp && k < num_rp + 2 * num_cp) // LHS comp poles, corresponding to imag part of residue for the pair
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in LHS comp 2 with pole_iter of" << pole_iter << std::endl;
            if (log_file)
              log_file.close();
            if (pole_write)
              pole_write.close();
        ...
        else if (k == num_rp + 2 * num_cp) // insert '1' for remainder
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in '1' column" << std::endl;
            if (log_file)
              log_file.close();
            if (pole_write)
              pole_write.close();
        ...
        else if (k > num_rp + 2 * num_cp && k <= num_rp + 2 * num_cp + num_rp) // RHS real poles
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in RHS real" << std::endl;
            if (log_file)
              log_file.close();
            if (pole_write)
              pole_write.close();
        ...
        else if (k > num_rp + 2 * num_cp + num_rp && k <= num_rp + 2 * num_cp + num_rp + num_cp) // RHS comp poles, corresponding to real part of residue for the pair
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in RHS comp 1 with pole_iter of " << pole_iter << std::endl;
            if (log_file)
              log_file.close();
            if (pole_write)
              pole_write.close();
        ...
        else if (k > 2 * num_rp + 3 * num_cp && k <= num_rp + 2 * num_cp + num_rp + 2 * num_cp) // RHS comp poles, corresponding to imag part of residue for the pair
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in RHS comp 2 with pole_iter of " << pole_iter << std::endl;
            if (log_file)
              log_file.close();
            if (pole_write)
              pole_write.close();
    ...
    for (int k = 0; k < num_sp; k++)
    {
      ...
      {
        if (log_file)
          log_file << "inf or NaN in index " << k << " in B matrix\nExiting run...";
        if (log_file)
          log_file.close();
        if (pole_write)
          pole_write.close();
    ...
    {
      if (log_file)
        log_file << "LAPACKE_dgelss failed on iteration << " << iteration << "with return value: " << info_soln << "\nExiting run...\n";
      if (pole_write)
        pole_write.close();
      if (log_file)
        log_file.close();
    ...
    if (iteration < iters)
    {
      ...
      for (int k = 0; k < num_rp; k++)
      {
        ...
        {
          if (log_file)
            log_file << "inf or Nan at index (" << k << ',' << k << ") of Az_real!\n";
          if (pole_write)
            pole_write.close();
          if (log_file)
            log_file.close();
        ...
        {
          if (log_file)
            log_file << "inf or Nan at index " << k << " of c_real!\n";
          if (pole_write)
            pole_write.close();
          if (log_file)
            log_file.close();
      ...
      if (num_rp > 0) // some params of cblas_dgemm must be at least 1, so if no real poles exist then this operation won't be valid and should skip it
      {
        ...
        {
          if (log_file)
            log_file << "LAPACKE_dgeev failed with real poles!\nExiting run...\n";
          if (pole_write)
            pole_write.close();
          if (log_file)
            log_file.close();
      ...
      if (num_cp > 0)
      {
        ...
        {
          if (log_file)
            log_file << "LAPACKE_dgeev failed with complex poles!\nExiting run...\n";
          if (pole_write)
            pole_write.close();
          if (log_file)
            log_file.close();
    ...
    else if (iteration == iters)
    {
      ...
      for (int j = 0; j < a_matrix_final_r; j++) // rows
      {
        for (int k = 0; k < num_poles; k++) // cols (only need to loop over num poles)
        {
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << k << " in a_matrix_final norm!\n Exiting run...\n";
            if (pole_write)
              pole_write.close();
            if (log_file)
              log_file.close();
          ...
          {
            if (log_file)
              log_file << "inf or NaN in index " << j << ", " << num_poles + k << " in a_matrix_final conj!\n Exiting run...\n";
            if (pole_write)
              pole_write.close();
            if (log_file)
              log_file.close();
  ...
  in_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
  if (log_file)
    log_file << std::ctime(&in_time) << "\tFinished Vector Fitting Algorithm! Writing out results...\n";
  ...
  if (vflog)
    results_write.open("results.txt", std::ios::app | std::ios::out);
  ...
  for (int k = 0; k < poles_guess.size(); k++)
  {
    ...
    if (results_write)
      results_write << poles_guess[k].real() << " + " << poles_guess[k].imag() << "i\n";
  }
  if (results_write)
  ...
  for (int k = 0; k < residues.size(); k++)
  {
    ...
    if (results_write)
      results_write << residues[k].real() << " + " << residues[k].imag() << "i\n";
  }
  if (results_write)
    results_write << "\n\t==============================\tREMAINDER OF FIT:\t======================\n"
                  << remainder << '\n';
  ...
  in_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
  if (log_file)
    log_file << std::ctime(&in_time) << "\tdo_vector_fitting() finished, it took " << time_to_run.count() << " seconds to run!\n\nClosing log file...\n";
  if (results_write)
    results_write.close();
  if (pole_write)
    pole_write.close();
