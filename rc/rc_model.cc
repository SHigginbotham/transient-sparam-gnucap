...
namespace
{
  ...
  bool RC_MODEL::param_is_printable(int I) const
  {
    ...
    {
    case 0:
      return nump.has_hard_value();
    case 1:
      return numi.has_hard_value();
    case 2:
      return vflog.has_hard_value();
    case 3:
      return rclog.has_hard_value();
    default:
      return STORAGE::param_is_printable(I);
    }
  }
  ...
  void RC_MODEL::set_param_by_index(int I, std::string &Value, int Offset)
  {
    ...
    case 3:
      ...
      break;
    default:
      STORAGE::set_param_by_index(I, Value, Offset);
      break;
    }
  }
  ...
  void RC_MODEL::tr_begin()
  {
    ...
    try
    {
      std::filesystem::remove("tr_write.txt");
    }
    catch (...)
    {
    }
    ...
    if (tr_data)
      tr_data << "Opening file in tr_begin()...\n";
  ...
  void RC_MODEL::tr_advance()
  {
    ...
    if (time_step == 1)
    {
      lambda_k_1.clear();
      mu_k_1.clear();
    }                          // 2nd time step (time_step == 1)
    res_lambda_sum = 0. * one; // set to 0
    if (time_step == 1)
      res_lambda1_sum = 0. * one;
    ...
    for (int i = 0; i < vf_poles.size(); i++) // # poles == # residues
    {
      ...
      if (time_step == 1)
      {
        lambda_k_1.push_back(lambda_1);
        mu_k_1.push_back(mu_1);
      }
      mu_k.push_back(mu);
      nu_k.push_back(nu);
    }
    ...
    // calculate i_c term
    if (time_step == 1)
      _i[0].f1 = 2 / (std::sqrt(Zref) * (1. + factor_1));
    else
      _i[0].f1 = 2 / (std::sqrt(Zref) * (1. + factor));
    _i[0].x = h_n;
    i_c = _i[0].f1 * _i[0].x;
